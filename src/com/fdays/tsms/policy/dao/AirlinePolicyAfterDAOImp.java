 ...
 * 2010-12-10
 */
public class AirlinePolicyAfterDAOImp extends BaseDAOSupport implements AirlinePolicyAfterDAO {
	...
	public void deleteById(long id) throws AppException {
		if (id > 0) {
			AirlinePolicyAfter airlinePolicyAfter = (AirlinePolicyAfter) this.getHibernateTemplate().get(
					AirlinePolicyAfter.class, new Long(id));
	...
	public AirlinePolicyAfter getAirlinePolicyAfterById(long id) {
		...
		try {
			if (id > 0) {
				...
				airlinePolicyAfter = (AirlinePolicyAfter) this.getHibernateTemplate().get(AirlinePolicyAfter.class,
						new Long(id));
	...
	public long merge(AirlinePolicyAfter airlinePolicyAfter)
			throws AppException {
	...
	public AirlinePolicyAfter queryById(long id) throws AppException {
		...
		try {
			if (query != null && query.list() != null
					&& query.list().size() > 0) {
				return (AirlinePolicyAfter) query.list().get(0);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	//更改对象
	public long update(AirlinePolicyAfter airlinePolicyAfter)
			throws AppException {
	...
	public List list(AirlinePolicyAfterListForm apalf) throws AppException {
		...
		if(apalf.getBeginDate() != null){										//日期
			...
			if(!"1970-01-01 00:00:00.0".equals(start)){							//不是1970，即有选择年
