...
public class TeamOperate
{
	...
	public String getTeamOperateHTML()
	{
		...
		// 待统计利润退票订单
		if (this.saleTranType == AirticketOrder.TRANTYPE_3
		    && this.saleStatus == AirticketOrder.STATUS_107)
		{
			if (uri.hasRight("sb71"))
			{
				if (uri.hasRight("sb71"))
		...
		// 待审核退票订单
		if (this.saleStatus == AirticketOrder.STATUS_116)
		{
		...
		/*
		 * if (this.buyStatus == AirticketOrder.STATUS_108) { if
		 * (this.buyBusinessType == AirticketOrder.BUSINESSTYPE__1) { MyLabel ml2 =
		 * new MyLabel(); StringBuffer sb = new StringBuffer();
		 * sb.append("onclick=\""); sb.append("showDiv11("); sb.append("'" +
		 * this.buyId + "',"); sb.append("'" +
		 * this.buyOrder.getIncomeretreatCharge() + "'"); sb.append(")\"");
		 * ml2.setEvents(sb.toString()); ml2.setLabText("[确认已付退款]");
		 * ml2.setEndText("<br/>"); myLabels.add(ml2);
		 * 
		 * operate.setMyLabels(myLabels); } }
		 */
		...
		if (this.saleStatus == AirticketOrder.STATUS_117)
		{
			if (this.buyBusinessType == AirticketOrder.BUSINESSTYPE__2)
			{
				if (uri.hasRight("sb78"))
				{
					MyLabel ml = new MyLabel();
					ml.setHref(this.path
					    + "/airticket/listAirTicketOrder.do?thisAction=checkTeamRefund&id="
					    + this.saleId);
					ml.setLabText("[审核退票]");
					ml.setEndText("<br/>");
					myLabels.add(ml);
		 ...
		 * new StringBuffer(); sb.append("onclick=\""); sb.append("showDiv101(");
		 * sb.append(this.buyId + ","); sb.append(this.orderGroup.getId() + ",");
		 * sb.append("'" + this.buyOrder.getAirOrderNo() + "',"); sb.append("'" +
		 * this.buyOrder.getTotalAmount() + "',"); sb.append("'" +
		 * this.buyOrder.getCyrs() + "',"); if(this.buyOrder.getAccount()!=null) {
		 * sb.append("'" + this.buyOrder.getAccount().getId()+ "',"); } else {
		 * sb.append("'" +0+ "',"); }
		 * 
		 * sb.append("'" + this.buyOrder.getOperate126Time() + "'");
		 * sb.append(")\""); ml2.setEvents(sb.toString());
		 * ml2.setLabText("[确认已支付]"); ml2.setEndText("<br/>"); myLabels.add(ml2);
		 * operate.setMyLabels(myLabels); }
		 * 
		 * if (this.buyTranType == 3 && buyStatus== AirticketOrder.STATUS_109 &&
		 * uri.hasRight("sb81") ) { MyLabel ml2 = new MyLabel(); StringBuffer sb =
		 * new StringBuffer(); sb.append("onclick=\""); sb.append("showDiv14(");
		 * sb.append("'" + this.buyOrder.getId() + "',"); sb.append("'" +
		 * this.buyOrder.getIncomeretreatCharge() + "',");
		 * if(this.buyOrder.getAccount()!=null) { sb.append("'" +
		 * this.buyOrder.getAccount().getId()+ "',"); } else { sb.append("'" +0+
		 * "',"); } sb.append("'" + this.buyOrder.getMemo() + "',"); sb.append("'" +
		 * this.buyOrder.getPayTime() + "'"); sb.append(")\"");
		 * ml2.setEvents(sb.toString()); ml2.setLabText("[确认已收退款]");
		 * myLabels.add(ml2); operate.setMyLabels(myLabels); }
		 */
