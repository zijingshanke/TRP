...
public class OrderStatementHqlUtil {
	public static Hql getOrderStatementHql(String orderIdExp)throws AppException{
		...
		System.out.println("==getOrderStatementHql2===========>>>>>>"+hql.getSql());		
		return hql;		
	}
	
	
	public static Hql getStatement(String orderIdExp,Long orderSubtype)throws AppException{
		Hql hql=new Hql();
		hql.add(" ( select ");
	
		if(orderSubtype==Statement.SUBTYPE_10){	
			hql.add("'<accountName>'"+"||"+" s.toAccount.name "+"||"+"'</accountName>'"+"||");
			hql.add("'<accountNo>'"+"||"+" s.toAccount.accountNo "+"||"+"'</accountNo>'"+"||");			
		}
		...
		if(orderSubtype==Statement.SUBTYPE_20){
			hql.add("'<accountName>'"+"||"+" s.fromAccount.name "+"||"+"'</accountName>'"+"||");
			hql.add("'<accountNo>'"+"||"+" s.fromAccount.accountNo "+"||"+"'</accountNo>'"+"||");		
		}		
