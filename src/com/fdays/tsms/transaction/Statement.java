...
public class Statement extends _Statement {
	...
	public String getOrderSubtypeText() {
		return getSubTypeInfoByValue(this.orderSubtype);
	}

	public static String getSubTypeInfoByValue(Long orderSubType) {
		if (orderSubType != null) {
			if (orderSubType == SUBTYPE_20) {
				return "正常付款";
			} else if (orderSubType == SUBTYPE_21) {
				return "付退款";
			} else if (orderSubType == SUBTYPE_10) {
				return "正常收款";
			} else if (orderSubType == SUBTYPE_11) {
				return "收退款";
			} else {
				return "";
			}
	...
	public String getFormatStatementDate() {
		if (this.statementDate != null) {
			return DateUtil.getDateString(this.statementDate,
					"yyyy-MM-dd HH:mm:ss");
		}
		return "";
	}
	...
	public String toLogString() {
		...
		if (this.getType() == Statement.type_2) // 支出
		{
			...
			temp = "结算单：" + this.getStatementNo() + "，结算时间是："
					...
					+ this.getTotalAmount() + "元，支出帐号为："
					+ this.getFromAccount().getName();
