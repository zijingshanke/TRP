	...
	public ActionForward updatePlatform(ActionMapping mapping, ActionForm form,
		...
		try {
			if (statement.getStatementId() > 0
					&& statement.getAirticketOrderId() > 0) {
				statement.setId(statement.getStatementId());
				Statement sment = statementBiz.getStatementById(statement
						.getId());// 根据ID查询结算表
				AirticketOrder airticketOrder = airticketOrderBiz
						.getAirticketOrderById(statement.getAirticketOrderId());// 根据ID查询机票订单表
				...
				if (actualAmount1 != null) {
					...
					if (actualAmount.compareTo(totalAmount) == 1
							|| actualAmount.compareTo(totalAmount) == 0) // 实收款>=总金额
					{
					...
					{
						BigDecimal unsettledAccount = unsettledAccount2
								.subtract(actualAmount1);// 未结金额=未结金额(原)-收回金额
															// (收回金额<=未结金额)
						sment.setActualAmount(actualAmount);
				...
				if (flag > 0) {
					...
					try {
						...
						ticketLog.setOptTime(new Timestamp(System
								.currentTimeMillis()));
