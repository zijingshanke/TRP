...
public class PlatComAccountListAction extends BaseAction {
	...
	public ActionForward savePage(ActionMapping mapping, ActionForm form,
		...
		try {
			// List<PaymentTool>
			// paymentToolList=paymentToolBiz.getPaymentToolList();//支付工具表
			// List<Platform> platformList
			// =platformBiz.getPlatformList();//交易平台表
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		try {
			for (int i = 0; i < platComAccountListForm.getSelectedItems().length; i++) {
				...
				int b = 0;
				if (id > 0)
					platComAccount = platComAccountBiz
							.getPlatComAccountById(id);// 查询子表中是否有数据
