...
<c:import url="../_jsp/mainTitle.jsp" charEncoding="UTF-8">
	...
	<c:param name="title2" value="对比银行/支付平台报表" />
</c:import>
		...
		<td valign="top" class="body">
			<table cellpadding="0" cellspacing="0" border="0">
				<tr>
					...
					<td>
						<html:select property="accountId" name="reportCompare"
							value="${tempCompare.accountId}" styleClass="colorblue2 p_5"
							style="width:150px;">
...
<script type="text/javascript">	
		function startBankCompare(){				   
		    document.forms[0].action="<%=path%>/transaction/reportCompare.do?thisAction=compareBankReport";
		    document.forms[0].submit();
		}		
		
		function addBankReport(){	
			if(checkForm()){   
		    	document.forms[0].action="<%=path%>/transaction/reportCompare.do?thisAction=insertBankReport";
		    	document.forms[0].submit();
			}			
		}
		
		function updateBankCompareList(){	
			if(checkForm()){		   
		    	document.forms[0].action="<%=path%>/transaction/reportCompare.do?thisAction=updateBankOrderCompareList";
		    	document.forms[0].submit();
			}			 
		}		
